...
const TemporalGraphVisualizer = () => {
  ...
  const cppBackend = {
    ...
    computeCentrality: (nodes, edges, currentTime) => {
      ...
      nodes.forEach((node) => {
        const reachable = cppBackend.temporalBFS(
          node,
          currentTime,
          nodes,
          edges
  ...
  useEffect(() => {
    ...
    const bfsNodes = bfsResult
      ? new Set(bfsResult.map((r) => r.node))
      : new Set();
  ...
  const runTemporalBFS = () => {
    if (selectedNode) {
      const result = cppBackend.temporalBFS(
        selectedNode,
        currentTime,
        nodes,
        edges
        ...
        <div className="w-80 bg-gray-800 bg-opacity-50 backdrop-blur rounded-xl p-4 overflow-y-auto space-y-4 shadow-2xl border border-gray-700">
          ...
          {selectedNode && (
            <div className="bg-purple-900 bg-opacity-50 p-4 rounded-lg border border-purple-500">
              ...
              <p className="text-2xl font-mono font-bold text-purple-300 mb-3">
                {selectedNode}
              </p>
              <button
            ...
            <div className="bg-pink-900 bg-opacity-50 p-4 rounded-lg border border-pink-500 max-h-48 overflow-y-auto">
              ...
              <div className="space-y-1 text-xs font-mono">
                {bfsResult.map((r, i) => (
                  <div
                    key={i}
                    className="bg-pink-800 bg-opacity-50 p-2 rounded"
                  >
                    {r.node} at t={r.time}
                  </div>
                ))}
